...
from .affine_utils import Rigid
...
def _normalize(tensor, dim=-1):
    ...
    normed_tensor = torch.where(normed_tensor == 0.0, torch.tensor(1.0, device=normed_tensor.device), normed_tensor)
...
class PIE(nn.Module):
    ...
    def forward(self, X, E_in_idx=None, E_ex_idx=None):
        atom_N, atom_Ca, atom_C, atom_O = X[:, :4, :].unbind(1)
