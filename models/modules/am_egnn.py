...
class AM_E_GCL(nn.Module):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, n_channel, channel_nf, radial_nf,
                 ...
                 normalize=False, coords_agg='mean', tanh=False, dropout=0.1, init=None):
...
def coord2radial(edge_index, coord, attr, channel_weights, linear_map):
    ...
    for i in range(NUM_SEG):
        ...
        radial_clamped = torch.clamp(radial, min=-1e8, max=1e8)
        radial_norm = torch.norm(radial_clamped, dim=-1, keepdim=True) + CONSTANT  # post norm
        radial = linear_map(radial) / radial_norm # [n_edge, d_out]
        radial[torch.isnan(radial)] = 0.0
