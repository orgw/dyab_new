...
class AM_E_GCL(nn.Module):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, n_channel, channel_nf, radial_nf,
                 ...
                 normalize=False, coords_agg='mean', tanh=False, dropout=0.1, init=False):
        ...
        input_edge = input_nf * 2
        if not init:
            self.pie_mlp = nn.Linear(3 * hidden_nf, hidden_nf)
            self.edge_mlp = nn.Sequential(
                nn.Linear(input_edge + radial_nf + edges_in_d + hidden_nf, hidden_nf),
                act_fn,
        ...
        self.kernel_size = 10
        self.Ws = nn.Parameter(torch.randn(self.kernel_size + 1))
        self.bs = nn.Parameter(torch.randn(self.kernel_size + 1))
    ...
    def forward(self, h, edge_index, coord, channel_attr, channel_weights,
                edge_attr=None, node_attr=None, H_sv=None, H_se=None):
        '''
        ...
        dist_diff = torch.clamp(torch.abs(row - col), max=self.kernel_size)
        W_dist = torch.index_select(self.Ws, 0, dist_diff.type(torch.long))
        b_dist = torch.index_select(self.bs, 0, dist_diff.type(torch.long))

        if not self.init:
            pie_feat = torch.cat([H_sv[row], H_sv[col], H_se], dim=1)
            edge_attr = self.pie_mlp(pie_feat)
            edge_attr = edge_attr * W_dist.unsqueeze(-1).expand_as(edge_attr) + b_dist.unsqueeze(-1).expand_as(edge_attr)
            edge_feat = self.edge_model(h[row], h[col], radial, edge_attr)
        else: 
            edge_feat = self.edge_model(h[row], h[col], radial, edge_attr)
            edge_feat = edge_feat * W_dist.unsqueeze(-1).expand_as(edge_feat) + b_dist.unsqueeze(-1).expand_as(edge_feat)
        coord = self.coord_model(coord, edge_index, coord_diff, edge_feat, channel_weights)
