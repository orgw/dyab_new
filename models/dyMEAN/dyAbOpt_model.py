...
class dyAbOptModel(nn.Module):
    def __init__(self, embed_size, hidden_size, n_channel, num_classes,
                 ...
                 cdr_type='H3', relative_position=False, fix_atom_weights=False, **kwargs) -> None:
        super().__init__()
    ...
    def message_passing(self, X, S, residue_pos, batch_id, t, smooth_prob=None, smooth_mask=None):
        # embeddings
        ...
        pre_H_0 = H_0.clone()
        H, pred_X = self.gnn(H_0 + time_emb, X, edges,
    ...
    def _forward(self, X, S, cmask, smask, residue_pos, init_noise=None, sample=False):
        ...
        if sample is False:
            ...
            pred_S_logits, pred_X, H = self.message_passing(pseudo_X, S, residue_pos, batch_id, t, pseudo_S[smask], smask)
            ...
            for t in np.linspace(0, 1, steps)[:-2]:
                ...
                pred_S_logits, pred_X, H = self.message_passing(X, S, residue_pos, batch_id, tt, S_prob[smask], smask)
    ...
    def forward(self, X, S, cmask, smask, residue_pos, lengths, xloss_mask, context_ratio=0, seq_alpha=1, **kwargs):
        '''
        ...
        # get results
        H, pred_S, r_pred_S_logits, pred_X, prmsd = self._forward(X, S, cmask, smask, residue_pos, sample=False)
    ...
    def sample(self, X, S, cmask, smask, residue_pos, lengths, return_hidden=False, init_noise=None):
        ...
        # generate
        H, pred_S, r_pred_S_logits, pred_X, _ = self._forward(X, S, cmask, smask, residue_pos, init_noise, sample=True)
...
if __name__ == '__main__':
    ...
    model = dyAbOptModel(embed_size, hidden_size, n_channel,
