...
class AF_Dataset(torch.utils.data.Dataset):
    def __init__(self, pdb, heavy_chain, light_chain, antigen_chains, num_residue_changes, cdr='H3', use_af2ag=True) -> None:
        super().__init__()
        self.pdb = pdb
        self.num_residue_changes = num_residue_changes
        self.cdr = cdr
        self.use_af2ag = use_af2ag
        
        cplx = AgAbComplex.from_pdb(pdb, heavy_chain, light_chain, antigen_chains)

        # If using AF2-predicted epitopes, replace the antigen epitope data
        if self.use_af2ag:
            with open('all_data/af2_monomer_pdbid.json', 'r') as f:
                af2ag_pdb_id = json.load(f)

            pdb_id = cplx.get_antigen().pdb_id[:4]
            if pdb_id in af2ag_pdb_id:
                antigen = Protein.from_pdb(f'all_data/AF2_antigen_aligned_pdbs/{pdb_id}.pdb')
                cplx.antigen = antigen
                cplx.epitope = cplx._cal_epitope()

        # Generate antigen data (updated for AF2-predicted epitopes if applicable)
        ag_residues = []
        ...
        data = {key: np.concatenate([ag_data[key], hc_data[key], lc_data[key]], axis=0) for key in hc_data}

        cmask = ([1 if self.use_af2ag else 0 for _ in ag_data['S']] +
                 [0] + [1 for _ in hc_data['S'][1:]] +
                 [0] + [1 for _ in lc_data['S'][1:]])
...
def optimize(
    ...
    if type(cdr_type) == str:
        cdr_type = [cdr_type]
    dataset = AF_Dataset(**cplx_summary.__dict__, num_residue_changes=num_residue_changes, cdr=cdr_type)
