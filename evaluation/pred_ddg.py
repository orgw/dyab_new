...
def foldx_dg(pdb_path, rec_chains, lig_chains):
    ...
    try:
        ...
        # Search for all occurrences of "Total" followed by an energy value
        total_pattern = r"Total\s+=\s+([-+]?[0-9]*\.?[0-9]+)"
        matches = re.findall(total_pattern, output)

        if not matches:
            raise ValueError("No Total energy found in FoldX output.")
    finally:
