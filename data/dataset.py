...
class E2EDataset(torch.utils.data.Dataset):
    def __init__(self, file_path, save_dir=None, cdr=None, paratope='H3', full_antigen=False, num_entry_per_file=-1, random=False, use_af2ag=False):
        '''
    ...
    def collate_fn(cls, batch):
        ...
        types = [torch.float, torch.long, torch.bool, torch.bool, torch.bool, torch.long, torch.float, torch.bool, torch.float]
        res = {}
...
class BaseData(pl.LightningDataModule):
    def __init__(self, train_loader=None, valid_loader=None, test_loader=None):
        super().__init__()
