...
class Protein:
    ...
    def rename_chain(self, old_id, new_id):
        if old_id in self.peptides:

            peptide = self.peptides.pop(old_id)

            peptide.set_id(new_id)

            self.peptides[new_id] = peptide
        else:
            print(f"Chain ID {old_id} not found in protein.")
...
class AgAbComplex:
    ...
    def __init__(self, antigen: Protein, antibody: Protein, heavy_chain: str, light_chain: str=None,
    ...
    @classmethod
    def from_noL_pdb(cls, pdb_path: str, heavy_chain: str,  light_chain: str, antigen_chains: List[str],
                 ref_pdb_path: str='',numbering: str='imgt', skip_epitope_cal=False, skip_validity_check=False):
        protein = Protein.from_pdb(pdb_path)
        pdb_id = protein.get_id()
        if ref_pdb_path != '':
            ref_protein = Protein.from_pdb(ref_pdb_path)
            ab_peptides = {
                heavy_chain: protein.get_chain('H'),
                light_chain: ref_protein.get_chain(light_chain)
            }
            ag_peptides = { chain: protein.get_chain(chain) for chain in ['A']}
        else:
            ab_peptides = {
                heavy_chain: protein.get_chain(heavy_chain),
                light_chain: ref_protein.get_chain(light_chain)
        ...
        antigen = Protein(pdb_id, ag_peptides)
        antibody = Protein(pdb_id, ab_peptides)
        antigen_chain_ids = antigen.get_chain_names()
        antibody_chain_ids = antibody.get_chain_names()
        

        if len(antigen_chains) == 1:
            # chain_ids = antigen.get_chain_names()
            current_chain_id = antigen_chain_ids[0]
            new_chain_id = antigen_chains[0]  
            if current_chain_id != new_chain_id:
                antigen.rename_chain(current_chain_id, new_chain_id)
            current_heavy_id = antibody_chain_ids[0]
            if current_heavy_id != heavy_chain:
                antibody.rename_chain(current_heavy_id, heavy_chain)
        return cls(antigen, antibody, heavy_chain, light_chain, numbering, skip_epitope_cal, skip_validity_check)

    @classmethod
    def from_antigen_pdb(cls, pdb: str, pdb_path: str, heavy_chain: str,  light_chain: str, antigen_chains: List[str],
                 numbering: str='imgt', skip_epitope_cal=False, skip_validity_check=False):
        protein = Protein.from_pdb(pdb_path)
        ag_path = f'./all_data/AF2_antigen_aligned_pdbs/{pdb}.pdb'
        ag = Protein.from_pdb(ag_path)
        pdb_id = protein.get_id()
        ...
        possible_chains = list(string.ascii_uppercase)  # ['A', 'B', 'C', ..., 'Z']

        ag_peptides = { chain: ag.get_chain(chain) for chain in possible_chains if ag.get_chain(chain) is not None }
        # for chain in antigen_chains:
        #     assert chain in ag_peptides, f'Antigen chain {chain} has something wrong!'
    ...
    @classmethod
    def from_seperate_pdb(cls, pdb:str, pdb_path: str, heavy_chain: str, light_chain: str, antigen_chains: List[str],
                 numbering: str='imgt', skip_epitope_cal=False, skip_validity_check=False):

        ag_path = f'./all_data/AF2_antigen_aligned_pdbs/{pdb}.pdb'
        protein = Protein.from_pdb(ag_path)
        pdb_id = protein.get_id()
        ab_peptides = {
            heavy_chain: protein.get_chain(heavy_chain),
            light_chain: protein.get_chain(light_chain)
    ...
    def get_antigen_chain(self) -> Peptide:
        return self.antibody.get_chain(self.antigen)
...
def merge_to_one_protein(protein1: Protein, protein2: Protein):
    residues = []
    chain_order1 = sorted(protein1.get_chain_names())
    chain_order2 = sorted(protein2.get_chain_names())
    for chain_name in chain_order1:
        chain1 = protein1.get_chain(chain_name)
        for _, residue in enumerate(chain1.residues):
            residue.id = (len(residues), ' ')
            residues.append(residue)
            
    for chain_name in chain_order2:
        chain2 = protein2.get_chain(chain_name)
        for _, residue in enumerate(chain2.residues):
            residue.id = (len(residues), ' ')
            residues.append(residue)
    return Protein(protein1.get_id(), {'A': Peptide('A', residues)}) 
