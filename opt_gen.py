...
def opt_generate(args):
    ...
    from models import dyAbOptITF

    model_itf = dyAbOptITF.load_from_checkpoint(args.ckpt)
    model = model_itf.model
    ...
    for item_id, item in enumerate(items):
        pdb_name = item['pdb']
        pdb_path = f'./all_data/pdb/{pdb_name}.pdb'
        if not os.path.exists(pdb_path):
            print(f"Skipping {pdb_name} as {pdb_path} does not exist.")
            continue 
...
def parse():
    ...
    # parser.add_argument('--use_af2ag', type=str, default=True, help='Use AF2 antigens')
